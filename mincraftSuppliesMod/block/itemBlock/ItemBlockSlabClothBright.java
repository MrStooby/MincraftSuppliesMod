...
public class ItemBlockSlabClothBright extends ItemBlock {

	private final boolean isFullBlock;
	private final Block theHalfSlab;
	private final Block doubleSlab;

	public ItemBlockSlabClothBright(int id) {
		super(id);
		this.theHalfSlab = MincraftSuppliesMod.singleSlabClothBright;
		this.doubleSlab = MincraftSuppliesMod.doubleSlabClothBright;

		if (id - 256 == MincraftSuppliesMod.doubleSlabClothBright.blockID) {
	...
	public String getUnlocalizedName(ItemStack stack) {
		return ((SlabClothBright) theHalfSlab).getFullSlabName(stack.getItemDamage());
	}
	...
	public boolean canPlaceItemBlockOnSide(World world, int x, int y, int z,
			int side, EntityPlayer player, ItemStack stack) {
		int i1 = x;
		int j1 = y;
		int k1 = z;
		...
		if ((side == 1 && !flag || side == 0 && flag)
				&& id == this.theHalfSlab.blockID
				&& j2 == stack.getItemDamage()) {
			return true;
		} else {
			if (side == 0) {
				--y;
			...
			j2 = meta & 7;
			flag = (meta & 8) != 0;
			return id == this.theHalfSlab.blockID
					&& j2 == stack.getItemDamage() ? true : super
					.canPlaceItemBlockOnSide(world, i1, j1, k1, side, player,
							stack);
		}
	}

	private boolean placeDoubleSlabFromTop(ItemStack stack,
